...
class DB_WeatherSubscribe:
    ...
    def insert_weather_info(self, user_id, city_code):
        ...
        # 插入数据
        cursor.execute(
            "INSERT OR REPLACE INTO WeatherSubscribe (qq_number, city_code, weather_condition, query_time) VALUES (?, ?, ?, ?)",
    ...
    def update_weather_info(self, weather_condition, query_time, qq_number):
        ...
        # 执行更新操作
        cursor.execute(
            "UPDATE WeatherSubscribe SET weather_condition = ?, query_time = ? WHERE qq_number = ?",
            (weather_condition, query_time, qq_number),
        )
        # 提交事务
        conn.commit()
        # 关闭Cursor
    ...
    def get_last_weather_info(self, city_code):
        ...
        cursor.execute(
            "SELECT weather_condition, query_time FROM WeatherSubscribe WHERE city_code = ? ORDER BY qq_number DESC LIMIT 1",
            (city_code,),
        )
    ...
    def find_people_in_db(self):
        ...
        cursor.execute(
            "SELECT city_code, GROUP_CONCAT(qq_number), GROUP_CONCAT(weather_condition), GROUP_CONCAT(query_time) FROM WeatherSubscribe GROUP BY city_code"
        )
        # 获取查询结果
...
class Weather_Dectector:
    ...
    async def get_weather_data(self, citycode=370881):  # 获取天气数据
        ...
        key = yaml.load(
            open("./scripts/WeatherSubscribe/config.yml", "r"), Loader=yaml.FullLoader
        )["WeatherAPI_KEY"]
        url = (
            "https://restapi.amap.com/v3/weather/weatherInfo?key="
            + str(key)
            + f"&city={str(citycode)}&extensions=base&output=json"
        )
        #print(url)
    ...
    async def handle_WeatherSubscribe_public_message(self):
        ...
        try:
            ...
            if "sub" in self.raw_message and self.raw_message.startswith("sub"):
                ...
                # 判断功能开关
                judgement = db_judgement().judgement(self.group_id)
                ...
                if msg_process:
                    ...
                    city_code = await Weather_Dectector().dict_find_classify_citycode(
                        city_name
                    )

                    if city_code == None:
                        content = (
                            f"[CQ:at,qq={self.user_id}] 未找到{city_name}的天气信息。"
                        )

                        await send_group_msg(self.websocket, self.group_id, content)
                    else:
                        content = (
                            f"[CQ:at,qq={self.user_id}] 已订阅{city_name}的天气信息。"
                        )

                        DB_WeatherSubscribe(self.group_id).insert_weather_info(
                            self.user_id, city_code
                        )
                        await send_group_msg(self.websocket, self.group_id, content)

                        logging.info(
                            f"{self.name}({self.user_id})订阅{city_name}的天气信息。"
                        )

            elif "unpub" in self.raw_message and self.raw_message.startswith("unpub"):
                # 取消订阅天气
            ...
            for filename in os.listdir(folder_path):
                if filename.endswith(".db"):  # 确保文件是.db格式
                    ...
                    if len(parts) > 1:
                        ...
                        logging.info(
                            f"File: {filename} does not contain a valid group ID"
                        )
        except Exception as e:
            logging.error(
                f"WeatherSubscribe_Error(Weather_Subscribe_sender.add_group_id): {e}"
            )


    async def send_weather_msg(self, websocket,number_qq):
        data = time.strftime(
        ...
        try:
            ...
            if information_total != []:  # 确保信息不为空
                ...
                for city_code in json_data["city_code"]:
                    ...
                    (
                        ...
                        windpower,
                        temperature,
                    ) = await Weather_Dectector().get_weather_data(city_code)

                    if weather_data == True:
                        # 遍历字典，为每个QQ号列表生成一个拼接字符串
                        num_list = DB_WeatherSubscribe(
                            group_id
                        ).get_last_weather_info(city_code)
                        # print(num_list)
                        ...
                        if num_list[0] != "雨雪" or num_list[1] != data:
                            ...
                            qq_string = "".join(
                                [f"[CQ:at,qq={qq}]" for qq in qq_list]
                            )

                            # 遍历查询到的天气信息，为每个QQ号添加天气信息

                            for qq in qq_list:
                                DB_WeatherSubscribe(group_id).update_weather_info(
            ...
            logging.error(
                f"WeatherSubscribe_Error(Weather_Subscribe_sender.send_weather_msg): {e}"
            )
...
class Timer_count:
    ...
    async def handle_WeatherSubscribe_task_main(self, websocket):
        ...
        list_num = list_group_id[:-1]
        for number_qq in list_num: # 删除最后的判断的表
            #print(number_qq)
            judgement = db_judgement().judgement(number_qq)
            if judgement == True:
                task_sender = await asyncio.create_task(
                    Weather_Subscribe_sender().send_weather_msg(websocket, number_qq)
                )
            else:
                pass
        list_group_id.clear()
...
async def handle_WeatherSubscribe_task_Msg(websocket, msg):
    msg_task = await asyncio.create_task(
        Handle_WeatherSubscribe(websocket, msg).handle_WeatherSubscribe_public_message()
    )


async def handle_WeatherSubscribe_task_Timer(websocket):
    timer_task = asyncio.create_task(
        Timer_count().handle_WeatherSubscribe_task_main(websocket)
    )
